...
final class FundingSchematicConvergenceAccess
{
    ...
    public static function canEdit(?User $user, int $enteredByUserId): bool
    {
        if (! self::canSubmit($user)) {
            return false;
        }

        return (int) $enteredByUserId === (int) $user->id;
    }

    public static function canDelete(?User $user, int $enteredByUserId): bool
    {
        if ($user?->role === 'state_admin') {
            return true;
        }

        return self::canEdit($user, $enteredByUserId);
    }
    ...
    private static function isAllowlistedUser(?User $user): bool
    {
        ...
        $ids = config('funding_schematic_convergence.submitter_user_ids', []);
        if ($ids !== [] && in_array((int) $user->id, $ids, true)) {
            return true;
        }

        $names = config('funding_schematic_convergence.submitter_names', []);
        if ($names === []) {
            return false;
        }
