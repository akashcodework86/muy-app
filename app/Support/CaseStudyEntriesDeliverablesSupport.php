...
final class CaseStudyEntriesDeliverablesSupport
{
    private const TABLE = 'case_study_entries';
    ...
    public static function breakdown(?Carbon $periodFrom, ?Carbon $periodTo): array
    {
        ...
        return self::buildBreakdown($rows, 'story_date', function ($row): array {
            return [
                'id' => (int) $row->id,
                'reference' => 'Story #'.$row->id,
                'applicant' => (string) ($row->incubatee_name ?: '—'),
                'district' => 'State',
                'hub' => '—',
                'service' => BrandingCommunicationOptions::storyTypeLabel((string) $row->story_type).' · '.($row->story_title ?: '—'),
                'spoc' => (string) ($row->submitted_by_name ?: '—'),
                'status' => 'Logged',
                'date' => self::formatDate($row->story_date),
            ];
        });
    }
    ...
    private static function baseQuery(?Carbon $periodFrom, ?Carbon $periodTo): Builder
    {
        ...
        self::applyPeriodFilter($query, 't.story_date', $periodFrom, $periodTo);
