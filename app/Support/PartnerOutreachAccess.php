...
final class PartnerOutreachAccess
{
    ...
    public static function canDelete(?User $user, MarketingPartnerOutreachEntry $row): bool
    {
    ...
    private static function isAllowlistedUser(?User $user): bool
    {
        ...
        $ids = config('partner_outreach.submitter_user_ids', []);
        if ($ids !== [] && in_array((int) $user->id, $ids, true)) {
            return true;
        }

        $names = config('partner_outreach.submitter_names', []);
        if ($names === []) {
            return false;
        }
