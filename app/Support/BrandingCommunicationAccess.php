...
final class BrandingCommunicationAccess
{
    ...
    public static function canDelete(?User $user, int $submittedByUserId): bool
    {
    ...
    private static function isAllowlistedUser(?User $user): bool
    {
        ...
        $ids = config('branding_communication.submitter_user_ids', []);
        if ($ids !== [] && in_array((int) $user->id, $ids, true)) {
            return true;
        }

        $names = config('branding_communication.submitter_names', []);
        if ($names === []) {
            return false;
        }
