...
class DistrictWorkshopSessionAttendanceController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if (! Schema::hasTable('district_workshop_sessions')) {
            return redirect()
                ...
                ->withErrors(['topic' => 'District workshop sessions table is missing. Please run migrations first.']);
        }
    ...
    public function show(Request $request, DistrictWorkshopSession $districtWorkshopSession): View
    {
        ...
        return view('staff.district-workshop-sessions.show', [
            ...
            'applicantSnapshots' => $this->enrichedApplicantSnapshots($districtWorkshopSession),
            'currentRole' => (string) $user->role,
    ...
    public function edit(Request $request, DistrictWorkshopSession $districtWorkshopSession): View
    {
        ...
        $incubatees = $this->onboardedIncubateesForDistrict((int) ($districtWorkshopSession->district_id ?: 0))
            ->values();
        ...
        return view('staff.district-workshop-sessions.edit', [
            ...
            'row' => $districtWorkshopSession,
            'incubatees' => $incubatees,
            'totalOnboardedCount' => $this->onboardedIncubateesCountForDistrict((int) ($districtWorkshopSession->district_id ?: 0)),
            'selectedIds' => collect((array) $districtWorkshopSession->selected_incubatee_ids)->map(fn ($id): int => (int) $id)->all(),
        ]);
    }
    ...
    public function update(Request $request, DistrictWorkshopSession $districtWorkshopSession): RedirectResponse
    {
        ...
        $districtId = (int) ($districtWorkshopSession->district_id ?: 0);
        $selectedIds = $this->normalizeSelectedIncubateeIds((array) $validated['selected_incubatees']);
        $snapshots = $this->snapshotsForSelectedIncubatees($districtId, $selectedIds);
        ...
        $districtWorkshopSession->topic = trim((string) $validated['topic']);
        ...
        $districtWorkshopSession->selected_incubatee_ids = $selectedIds->all();
        $districtWorkshopSession->selected_incubatees_snapshot = $snapshots->all();
     ...
     * @return list<array<string, mixed>>
     */
    private function enrichedApplicantSnapshots(DistrictWorkshopSession $session): array
    {
