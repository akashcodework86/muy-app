...
class CaseStudyEntryLandingController extends Controller
{
    public function __invoke(Request $request): RedirectResponse
    {
        ...
        if ($request->user()?->role === 'state_staff' && BrandingCommunicationAccess::canSubmit($request->user())) {
            return redirect()->route('spoc.case-study-entries.create');
        }

        if ($request->user()?->role === 'state_admin') {
            return redirect()->route('admin.case-study-entries.dashboard');
        }

        return redirect()->route('spoc.case-study-entries.dashboard');
    }
}
