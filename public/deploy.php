...
echo "--- git pull ---\n";
$lines = [];
$run($projectPath, '/usr/bin/git reset --hard origin/master', $lines, $e1);
$out(implode("\n", $lines));
$run($projectPath, '/usr/bin/git pull origin master', $lines, $gitExit);
$out(implode("\n", $lines));
if ($gitExit !== 0) {
    echo "\n<span style='color:#f87171;font-weight:bold'>❌ git pull failed (exit {$gitExit}).</span>\n";
    echo '</pre>';
    exit;
}
...
$out(implode("\n", $lines));
if ($composerExit !== 0) {
    echo "\n<span style='color:#fbbf24;font-weight:bold'>⚠️ composer install failed (exit {$composerExit}). Continuing to migrations…</span>\n\n";
}
...
$out(implode("\n", $lines));
if ($migrateExit !== 0) {
    echo "\n<span style='color:#f87171;font-weight:bold'>❌ MIGRATIONS FAILED (exit {$migrateExit}). Database was NOT fully updated.</span>\n";
    echo "\n<span style='color:#94a3b8'>Tip: duplicate column on `services` → pull latest; migration 2026_04_28_100000 is idempotent.</span>\n";
    echo '</pre>';
    exit;
}
...
$healthScript = $projectPath.'/public/social-media-health.php';
if (is_file($healthScript)) {
    echo "Health URL: /social-media-health.php?key=akash\n";
} else {
    echo "Health URL: unavailable (upload public/social-media-health.php)\n";
}
