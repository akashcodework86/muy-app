...
@section('content')
    @php
        $stageTabs = [
            '' => 'All stages ('.number_format($stageCounts['all'] ?? 0).')',
    ...
    <div class="op-shell">
        ...
        <form method="get" action="{{ route($routePrefix.'.index') }}" class="op-toolbar">
            ...
            <a href="{{ route($routePrefix.'.export', array_merge($queryBase, ['stage' => $activeStage ?: null])) }}" class="op-btn" style="margin-left:auto;">Download CSV</a>
        </form>

        <div class="op-tabs">
            @foreach ($stageTabs as $key => $label)
                <a href="{{ route($routePrefix.'.index', array_merge($queryBase, ['stage' => $key ?: null])) }}" class="op-tab @if ($activeStage === $key) is-active @endif">{{ $label }}</a>
            @endforeach
        </div>
        ...
        <div class="op-table-wrap">
            <table class="op-table">
                ...
                <tbody>
                    @forelse ($rows as $row)
                        @php $showUrl = $showUrlResolver($row['id']); @endphp
                        <tr>
                            <td>{{ $row['rank'] }}</td>
                            <td>
                            ...
                            <td>
                                @if ($showUrl)
                                    <a href="{{ $showUrl }}" class="op-btn op-btn--secondary" style="padding:.3rem .55rem;font-size:.76rem;">View CFA</a>
                                @endif
                            </td>
                        </tr>
                    @empty
