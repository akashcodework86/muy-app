@php
    $ins = $insights ?? [];
    $chartPrimary = ($dashboardTheme ?? 'revamp') === 'legacy' ? '#d04a02' : '#1e40af';
    $chartSecondary = ($dashboardTheme ?? 'revamp') === 'legacy' ? '#eb8c00' : '#0f766e';
    $chartFill = ($dashboardTheme ?? 'revamp') === 'legacy' ? 'rgba(208, 74, 2, 0.24)' : 'rgba(30, 64, 175, 0.18)';
@endphp
<script>
(function () {
    ...
    const districtPalette = @json(($dashboardTheme ?? 'revamp') === 'legacy'
        ? ['#d04a02', '#eb8c00', '#ffb600', '#2d2d2d', '#464646', '#a63d02', '#6b6b6b', '#22c55e', '#c75b12', '#b83d02', '#8b8b8b', '#d97706', '#16a34a']
        : ['#1e40af', '#0f766e', '#0369a1', '#4338ca', '#0891b2', '#334155', '#15803d', '#7c3aed', '#2563eb', '#0d9488', '#64748b', '#ca8a04', '#16a34a']
    );
