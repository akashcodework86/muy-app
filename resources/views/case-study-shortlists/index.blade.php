                        ...
                        <fieldset class="css-proposal">
                            ...
                            <div class="css-proposal__options">
                                @foreach($nominationServices as $serviceCode => $serviceOption)
                                    <label class="css-proposal__option @if($serviceCode === 'other') css-proposal__other @endif">
                                    ...
                                    @if($serviceCode === 'other')
                                        <div class="css-proposal__other css-proposal__other-input">
                                            <input class="css-input" type="text" name="other_service" maxlength="255" placeholder="Enter proposed service name">
                                        </div>
                                    @endif
                                @endforeach
                            </div>
    ...
    <section class="css-card" id="case-study-shortlist-live-section">
        ...
        @forelse($rows as $row)
            <tr>
                ...
                <td style="min-width:210px"><div class="css-remarks">@forelse($deliveredServicesByShortlist->get($row->id, collect()) as $service)<div class="css-remark"><strong>{{ $service['name'] ?: 'Service' }}</strong><div class="css-muted">{{ $service['status'] ?: 'Recorded' }}@if($service['date']) &middot; {{ $service['date'] }}@endif</div>@foreach($service['documents'] ?? [] as $document)@php $documentUrl=$document['url']??route($routePrefix.'.documents.download',array_filter([$row,$document['type']??'',$document['id']??0,'index'=>$document['index']??null],fn($value)=>$value!==null)); @endphp<a class="css-doc" href="{{ $documentUrl }}" target="_blank" rel="noopener">&#128196; {{ $document['label'] ?? 'View document' }}</a>@endforeach</div>@empty<span class="css-muted">No delivered service recorded.</span>@endforelse</div></td>
