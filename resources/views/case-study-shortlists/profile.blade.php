        ...
        <section class="csp-card">
            ...
            <form method="post" action="{{ route($routePrefix.'.nominations.update',$shortlist) }}">@csrf @method('PUT')
                @foreach($nominationServices as $code=>$option)
                    ...
                    @if($code === 'other')
                        <input class="csp-textarea" style="min-height:44px;margin:-.25rem 0 .6rem" type="text" name="other_service" maxlength="255" value="{{ old('other_service', $active?->custom_service_name) }}" placeholder="Enter proposed service name">
                    @endif
                @endforeach
            ...
            @else
                @forelse($activeNominations as $nomination)<div class="csp-option"><strong>{{ $nominationServices[$nomination->service_code]['label'] ?? ucfirst($nomination->service_code) }}@if($nomination->service_code === 'other' && $nomination->custom_service_name): {{ $nomination->custom_service_name }}@endif</strong><div class="csp-tags"><span class="csp-tag">{{ str_replace('_',' ',ucfirst($nomination->status)) }}</span></div><p>{{ $nomination->nomination_note }}</p></div>@empty<p class="csp-muted">No active service nominations yet.</p>@endforelse
            @endif
        </section>

        <section class="csp-card"><h3 class="csp-title">Nomination audit</h3>@forelse($shortlist->nominations as $nomination)<div class="csp-audit"><strong>{{ $nominationServices[$nomination->service_code]['label'] ?? ucfirst($nomination->service_code) }}@if($nomination->service_code === 'other' && $nomination->custom_service_name): {{ $nomination->custom_service_name }}@endif</strong><div class="csp-muted">Current: {{ str_replace('_',' ',ucfirst($nomination->status)) }}</div>@foreach($nomination->events as $event)<div class="csp-muted" style="margin-top:.35rem">{{ ucfirst($event->action) }} by {{ $event->actor?->name ?: 'State Admin' }}<br>{{ $event->created_at?->format('d M Y, h:i A') }}</div>@endforeach</div>@empty<p class="csp-muted">No nomination activity yet.</p>@endforelse</section>
