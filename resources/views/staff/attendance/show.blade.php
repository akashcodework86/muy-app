...
<div class="ws-shell">
    ...
    <div class="ws-breadcrumb">
        <a href="{{ route('staff.attendance.view') }}"><i class="fa-solid fa-table-list"></i> Workshop records</a>
    ...
    <div class="ws-banner">
        ...
        <div class="ws-banner__actions">
            ...
            @endif
            <a href="{{ route('staff.attendance.view') }}" class="ws-btn ws-btn--ghost ws-btn--sm">
        ...
        <div class="ws-card__body">
            @if (count($mediaItems) > 0)
                <div class="ws-photo-grid">
                    @foreach ($mediaItems as $idx => $item)
                        <a href="{{ route('staff.attendance.attachment', ['attendanceReport' => $report, 'index' => $idx, 'inline' => 1]) }}"
                           target="_blank" rel="noopener" class="ws-photo-item">
                            <img src="{{ route('staff.attendance.attachment', ['attendanceReport' => $report, 'index' => $idx, 'inline' => 1]) }}"
            ...
            @elseif ($report->attachment_path)
                <div class="ws-photo-empty">
                    <a href="{{ route('staff.attendance.attachment', $report) }}" class="ws-btn ws-btn--teal ws-btn--sm">
        ...
        <div class="ws-card__body">
            @if ($report->hasAttendanceSheet())
                <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">
                    ...
                    <a href="{{ route('staff.attendance.sheet.download', $report) }}" class="ws-btn ws-btn--teal ws-btn--sm">
            ...
            @else
                <p style="color:var(--ws-muted);font-size:0.88rem;margin:0;">
                    ...
                    @if ((int) $report->participants_total > 0 && (int) $report->field_coordinator_user_id === (int) auth()->id())
                        <a href="{{ route('staff.attendance.sheet-template.report', $report) }}" class="ws-btn ws-btn--ghost ws-btn--sm" style="margin-left:0.75rem;">
                            <i class="fa-solid fa-download"></i> Download template
                        </a>
                    @endif
                </p>
                @if ((int) $report->participants_total > 0 && (int) $report->field_coordinator_user_id === (int) auth()->id())
                    <form method="post" action="{{ route('staff.attendance.sheet.upload', $report) }}" enctype="multipart/form-data" style="display:flex;gap:0.5rem;align-items:center;margin-top:0.75rem;flex-wrap:wrap;">
                        @csrf
        ...
        <div class="ws-card__head">
            ...
            <div class="ws-card__head-actions">
                @if (count($participantRows) > 0)
                    <a href="{{ route('staff.attendance.participants.export', $report) }}" class="ws-btn ws-btn--teal ws-btn--sm">
            ...
            @else
                <div class="ws-empty-row" style="padding:2.5rem 1.4rem;">
                    ...
                    @if ($report->isDraft() && (int) $report->field_coordinator_user_id === (int) auth()->id())
                        <br>
                        <a href="{{ route('staff.attendance.index', ['draft' => $report->id]) }}" class="ws-btn ws-btn--indigo ws-btn--sm" style="margin-top:0.65rem;">
