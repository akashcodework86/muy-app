...
@section('title', 'Edit Field Visit Report')
@section('heading', 'Edit Field Visit Report')
...
<style>
    ...
    .fr-existing-attachment a { color: var(--fr-teal); font-weight: 600; text-decoration: none; }
    .fr-existing-attachment a:hover { text-decoration: underline; }
    .fr-remove-check { display: flex; align-items: center; gap: .4rem; font-size: .78rem; color: #dc2626; cursor: pointer; margin-top: .35rem; }
    .fr-remove-check input { accent-color: #dc2626; width: 14px; height: 14px; }
    ...
    .fr-file-area__hint { font-size: .7rem; color: #94a3b8; margin-top: .2rem; }
    .fr-file-name { margin-top: .4rem; font-size: .78rem; font-weight: 600; color: var(--fr-teal); }
...
<div class="fr-shell">
    <div class="fr-card">
        ...
        <h2 class="fr-card-title">
            ...
            Edit Visit Report
            <span class="fr-edit-badge">{{ $fieldReport->visit_date?->format('d M Y') }}</span>
        </h2>

        <form method="POST" action="{{ route('staff.field-reports.update', $fieldReport) }}" enctype="multipart/form-data" id="frForm">
            @csrf
            @method('PUT')
            ...
            <div class="fr-grid-2">
                ...
                <div class="fr-field">
                    ...
                    <input
                        ...
                        value="{{ old('visit_date', $fieldReport->visit_date?->toDateString()) }}"
            ...
            <div class="fr-grid-2">
                <div class="fr-field">
                    ...
                    >
                        ...
                        @foreach ($districts as $d)
                            <option value="{{ $d->id }}" @selected(old('district_id', $fieldReport->district_id) == $d->id)>{{ $d->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="fr-field">
                    ...
                    <select id="block_id" name="block_id" class="fr-select">
                        ...
                        @foreach ($blocks as $b)
                            <option value="{{ $b->id }}" @selected(old('block_id', $fieldReport->block_id) == $b->id)>{{ $b->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="fr-grid-1">
                <div class="fr-field">
                    ...
                    <input
                        ...
                        name="area"
                        class="fr-input"
                        value="{{ old('area', $fieldReport->area) }}"
            ...
            <div class="fr-grid-2">
                <div class="fr-field">
                    ...
                    <input
                        ...
                        value="{{ old('total_villages', $fieldReport->total_villages) }}"
                ...
                <div class="fr-field" style="grid-column:1/-1;">
                    ...
                    <textarea
                        ...
                        maxlength="2000"
                    >{{ old('village_names', $fieldReport->village_names) }}</textarea>
            ...
            <div class="fr-grid-2">
                <div class="fr-field">
                    ...
                    <input
                        ...
                        value="{{ old('total_participants', $fieldReport->total_participants) }}"
                ...
                <div class="fr-field">
                    ...
                    <input
                        ...
                        value="{{ old('outreach_programmes', $fieldReport->outreach_programmes) }}"
                        min="0" max="500" required
                    >
                ...
                <div class="fr-field">
                    ...
                    <input
                        ...
                        value="{{ old('cfas_reported', $fieldReport->cfas_reported) }}"
                        min="0" max="500" required
                    >
                    <div class="fr-sublabel">System will re-verify this against your referral records on save.</div>
            ...
            @if ($fieldReport->attachment_path)
                <div class="fr-existing-attachment">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0; color:#15803d;"><path d="M14 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8l-5-5Z"/><path d="M14 3v5h5"/></svg>
                    <span>Current file: <a href="{{ Storage::disk('public')->url($fieldReport->attachment_path) }}" target="_blank">{{ $fieldReport->attachment_original_name ?? 'View document' }}</a></span>
                </div>
                <label class="fr-remove-check">
                    <input type="checkbox" name="remove_attachment" value="1" @checked(old('remove_attachment'))>
                    Remove current attachment
                </label>
                <div style="margin-top:.75rem;">
                    <label class="fr-file-area" for="attachment" style="padding:.85rem;">
                        <input type="file" id="attachment" name="attachment" accept=".jpg,.jpeg,.png,.pdf" onchange="showFileName(this)">
                        <div class="fr-file-area__text"><strong>Upload a replacement</strong> (optional)</div>
                        <div class="fr-file-area__hint">JPG, PNG or PDF · max 5 MB</div>
            ...
            @else
                <label class="fr-file-area" for="attachment">
                    <input type="file" id="attachment" name="attachment" accept=".jpg,.jpeg,.png,.pdf" onchange="showFileName(this)">
                    <div style="color:var(--fr-muted); margin-bottom:.35rem;">
                        <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block; vertical-align:middle;">
            ...
            <div class="fr-actions">
                ...
                <a href="{{ route('staff.field-reports.index') }}" class="fr-back-link">← Cancel</a>
            </div>
        </form>
    </div>
</div>
