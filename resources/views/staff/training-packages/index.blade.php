...
@section('title', 'Training Package Attendance Dashboard')
@section('heading', 'Training Package Attendance Dashboard')
...
@section('content')
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('staff.training-packages.export', request()->query()) }}"
                    ...
                    <a href="{{ route('staff.training-packages.create') }}"
                        class="inline-flex items-center rounded-md bg-slate-900 px-3 py-2 text-xs font-medium text-white hover:bg-slate-700">
                        + Submission Form
                    </a>
                </div>
            </div>
            <form method="get" id="staffTpFilterForm" class="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-6">
                ...
                <a href="{{ route('staff.training-packages.index') }}" class="inline-flex items-center justify-center rounded-md border border-slate-300 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100">Clear</a>
            </form>
        </div>
            ...
            <table class="min-w-full text-left text-sm">
                <thead>
                    <tr class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                        ...
                        <th class="whitespace-nowrap px-4 py-3 font-semibold">Event Taken By</th>
                        ...
                        <th class="whitespace-nowrap px-4 py-3 font-semibold">Selected Incubatees</th>
                    ...
                    @forelse ($rows as $row)
                        <tr class="border-t border-slate-100 text-slate-700">
                            ...
                            <td class="whitespace-nowrap px-4 py-3">
                                <a href="{{ route('staff.training-packages.show', $row) }}" class="text-indigo-600 hover:text-indigo-800">View</a>
                                <span class="mx-1 text-slate-300">|</span>
                                <a href="{{ route('staff.training-packages.edit', $row) }}" class="text-slate-700 hover:text-slate-900">Edit</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    <script>
        (function () {
            const form = document.getElementById('staffTpFilterForm');
            if (!form) return;
