...
@section('content')
    <div style="max-width:55rem;">
        ...
        @if (! $wipeConfigured)
            <p style="background:#fef9c3; border:1px solid #fde047; color:#854d0e; padding:0.75rem 1rem; border-radius:8px; font-size:0.88rem;">
                This page is disabled until you set <strong>PROGRAMME_WIPE_SECRET</strong> in <code>.env</code> (at least 8 characters), then run
                <code>php artisan config:clear</code> (or <code>config:cache</code>) on the server.
            </p>
        @else
            @if (session('status'))
            ...
            <form method="POST" action="{{ route('admin.programme-structure-wipe.store') }}" style="background:#fff; border:1px solid #e4e4e7; border-radius:12px; padding:1rem 1.1rem;">
                ...
                <input type="text" name="confirm_phrase" value="{{ old('confirm_phrase') }}" required
                    autocomplete="off"
                    style="width:100%; max-width:28rem; padding:0.45rem 0.6rem; border:1px solid #d4d4d8; border-radius:6px; font-size:0.9rem; margin-bottom:0.85rem;">
