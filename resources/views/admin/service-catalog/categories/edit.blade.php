...
@section('content')
    ...
    <form method="post" action="{{ route('admin.service-catalog.categories.update', $category) }}" style="max-width:32rem;">
        ...
        <div style="margin-bottom:0.85rem;">
            ...
            @php $tm = old('target_mode', $category->target_mode ?? 'service'); @endphp
            <select id="target_mode" name="target_mode" style="width:100%; padding:0.45rem 0.5rem; border:1px solid #d4d4d8; border-radius:6px;">
                <option value="service" @selected($tm === 'service')>Service-wise targets (existing behavior)</option>
                <option value="category" @selected($tm === 'category')>Category-wise collective target</option>
            </select>
            <p style="font-size:0.78rem; color:#71717a; margin:0.3rem 0 0;">
                Switching mode updates target deliverable mapping for services in this category.
            </p>
