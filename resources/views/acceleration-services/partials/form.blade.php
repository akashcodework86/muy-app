...
@endphp
<div class="accel-card" id="accel-form">
    <h3 style="margin:0 0 0.35rem; font-size:1rem; font-weight:700;">New acceleration entry</h3>
    <p style="margin:0 0 1rem; font-size:0.82rem; color:#64748b;">Phase 1 applicants only. Select services ticked today; remarks and photos are optional.</p>
        ...
        <div class="accel-grid">
            <div>
                <div class="accel-field">
                    <label for="service_date">1. Service date <span style="color:#e11d48">*</span></label>
                    <input type="date" id="service_date" name="service_date" value="{{ old('service_date', now()->toDateString()) }}" required>
                </div>

                <div class="accel-field">
                    <label for="accel_search">2. Search Phase 1 applicant <span style="color:#e11d48">*</span></label>
                    <input type="text" id="accel_search" placeholder="Search by name, application no, or phone (min 2 characters)" autocomplete="off" @if($prefill) disabled @endif>

                    <div class="accel-picker" id="accel_picker">
                        <div class="accel-picker__grid">
                            <div>
                                <p style="margin:0 0 0.45rem; font-size:0.76rem; font-weight:700; color:#64748b; text-transform:uppercase; letter-spacing:0.05em;">Applicants</p>
                                <div id="accel_search_results" class="accel-picker__list">
                                    <p class="accel-picker__detail-empty" style="padding:0.75rem;">Type in the search box to load applicants.</p>
                                </div>
                            </div>
                    ...
                    <div id="accel_selected_applicant" class="accel-selected" style="{{ $prefill ? '' : 'display:none;' }}">
                        @if ($prefill)
                            ...
                            <br><span style="color:#64748b;">CFA: {{ $prefill['application_date'] ?? '—' }} · {{ $prefill['district_name'] ?: '—' }} · {{ $prefill['phone'] ?: '—' }} · {{ $prefill['onboard_label'] ?: '—' }}</span>
                        @endif
                    </div>
                ...
                @foreach ($sections as $sectionKey => $meta)
                    <p class="accel-section-title">{{ $meta['title'] }}</p>
                    @foreach ($catalog[$sectionKey] ?? [] as $item)
                        ...
                        <div class="accel-item" data-item-key="{{ $key }}">
                            <div class="accel-item__head">
                                ...
                                <label for="item_{{ $key }}" style="font-size:0.86rem; font-weight:600; cursor:pointer;">{{ $item['label'] }}</label>
                            </div>
                            <div class="accel-item__extra">
                                <div class="accel-field" style="margin-top:0.45rem;">
                                    <label>Remarks (optional)</label>
                                    <textarea name="remarks[{{ $key }}]" rows="2" placeholder="Optional notes"></textarea>
                                </div>
                                <div class="accel-field">
                    ...
                    <div class="accel-custom-row">
                        <input type="text" name="{{ $meta['custom'] }}[]" placeholder="+ Add new (saved for next time)">
                    </div>
                @endforeach
...
(function () {
    ...
    function renderSelectedApplicant(app) {
        ...
        selectedPanel.innerHTML = '<strong>Selected:</strong> ' + (app.applicant_name || 'Unnamed')
            ...
            + '<br><span style="color:#64748b;">' + applicantBriefMeta(app) + '</span>';
    }
    ...
    function renderApplicantList(list) {
        ...
        if (!list.length) {
            searchResults.innerHTML = '<p class="accel-picker__detail-empty" style="padding:0.75rem;">No applicants found.</p>';
            renderDetail(null);
            return;
        }
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function () {
            ...
            focusedId = null;
            if (q.length < 2) {
                searchResults.innerHTML = '<p class="accel-picker__detail-empty" style="padding:0.75rem;">Type at least 2 characters to search.</p>';
                renderDetail(null);
                return;
            }
            searchTimer = setTimeout(() => {
                searchResults.innerHTML = '<p class="accel-picker__detail-empty" style="padding:0.75rem;">Searching…</p>';
